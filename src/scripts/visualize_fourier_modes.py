...
def obtain_freq_vars(x, x_pred, idx):
    ...
    amplitude_mean = spectrum[:, -1, :].mean(dim=0).numpy()
    amplitude_std = spectrum[:, -1, :].std(dim=0).numpy()
    amplitude_mean_pred = spectrum_pred[:, -1, :].mean(dim=0).numpy()
    amplitude_std_pred = spectrum_pred[:, -1, :].std(dim=0).numpy()
    N = spectrum.shape[-1]
...
def one_step_plot_spectrum(pred, target, vars, model_name, batch_id = None, add_ribbon_bar=False):
    ...
    plt.savefig(f"../plot/spectrum_plot_{model_name}_{batch_id}.png")
