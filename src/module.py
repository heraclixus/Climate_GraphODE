...
class GlobalForecastModule(LightningModule):
    ...
    def train_step(self, batch):
        ...
        loss_dict = l2loss_sphere(solver=self.net.sw_solver, prd=pred, tar=target, vars=self.net.vars, lat=self.net.lat)
        return loss_dict 
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        ...
        all_loss_dicts = [m(preds, y, self.denormalization, out_variables, self.net.lat, self.val_clim, log_postfix) for m in [lat_weighted_mse_val, lat_weighted_rmse, lat_weighted_acc]]
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        # )
        preds = self.eval_step(batch)
        all_loss_dicts = [m(preds, y, self.denormalization, out_variables, self.net.lat, self.test_clim, log_postfix) for m in [lat_weighted_mse_val, lat_weighted_rmse, lat_weighted_acc]]
